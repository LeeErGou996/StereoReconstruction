...
bool estimatePose(const std::vector<cv::Point2f>& ptsL,
    ...
    std::string method = Config::instance().eightPointMethod;
    cv::Mat E;
    if (method == "opencv") {
        E = cv::findEssentialMat(ptsL, ptsR, K, cv::RANSAC);
    } else if (method == "hartley") {
        E = computeEssentialMatrix8Point(ptsL, ptsR, K);
    } else if (method == "classic") {
        E = computeEssentialMatrix8PointSimple(ptsL, ptsR, K);
    } else {
        E = cv::findEssentialMat(ptsL, ptsR, K, cv::RANSAC);
    }
...
cv::Mat computeEssentialMatrix8Point(
...
cv::Mat computeEssentialMatrix8PointSimple(
    const std::vector<cv::Point2f>& ptsL,
    const std::vector<cv::Point2f>& ptsR,
    const cv::Mat& K
) {
