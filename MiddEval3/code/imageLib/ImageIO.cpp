                 ...
                 int *width, int *height, int *nbands, int thirdArg)
{
	...
	if (fscanf(fp, "%d", width) != 1)
		throw CError("ReadFilePGM: could not read width");
	skip_space(fp);
	if (fscanf(fp, "%d", height) != 1)
		throw CError("ReadFilePGM: could not read height");
	if (thirdArg) {
		skip_space(fp);
		if (fscanf(fp, "%d", nbands) != 1)
			throw CError("ReadFilePGM: could not read nbands");
	}
...
void ReadFilePFM(CFloatImage& img, const char* filename)
{
    ...
    float scalef;
    if (fscanf(fp, "%f", &scalef) != 1)  // scale factor (if negative, little endian)
        throw CError("ReadFilePFM: could not read scale factor");
