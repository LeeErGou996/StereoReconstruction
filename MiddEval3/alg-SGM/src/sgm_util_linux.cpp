...
* Describe	: implement of sgm_util (Linux version)
*/
	...
	const sint32& height)
{
	...
	for (sint32 i = 2; i < height - 2; i++) {
		for (sint32 j = 2; j < width - 2; j++) {
			...
			// 窗口大小为5x5的窗口内逐像素比较，比一比较像素值与中心像素值的大小关系，组成census值
...
void sgm_util::census_transform_9x7(const uint8* source, uint64* census, const sint32& width, const sint32& height)
{
	...
	for (sint32 i = 4; i < height - 4; i++) {
		for (sint32 j = 3; j < width - 3; j++) {
			...
			// 窗口大小为9x7的窗口内逐像素比较，比一比较像素值与中心像素值的大小关系，组成census值
	...
	const sint32& p1, const sint32& p2_init, const uint8* cost_init, uint8* cost_aggr, bool is_forward)
{
	...
	// 正向(左->右) 令is_forward = true ; direction = 1
	// 反向(右->左) 令is_forward = false; direction = -1;
	...
	for (sint32 i = 0u; i < height; i++) {
		...
		// 路径上上一个像素的代价数组，多两个元素是为了避免边界溢出（首尾各多一个）
	...
	const uint8* cost_init, uint8* cost_aggr, bool is_forward)
{
	...
	// 正向(上->下) 令is_forward = true ; direction = 1
	// 反向(下->上) 令is_forward = false; direction = -1;
	const sint32 direction = is_forward ? 1 : -1;
	...
	const uint8* cost_init, uint8* cost_aggr, bool is_forward)
{
	...
	// 正向(左上->右下) 令is_forward = true ; direction = 1
	// 反向(右下->左上) 令is_forward = false; direction = -1;
	const sint32 direction = is_forward ? 1 : -1;
	...
	for (sint32 j = 0; j < width; j++) {
		...
		// 对角线路径上的下一个像素，隔width+1个像素
		// 这里要做一个边界处理
		// 特对角线前进时候会遇到影像行边界，则像素行号加一，原本的像素行号到了上一行边缘
	...
	const uint8* cost_init, uint8* cost_aggr, bool is_forward)
{
	...
	// 正向(右上->左下) 令is_forward = true ; direction = 1
	// 反向(左下->右上) 令is_forward = false; direction = -1;
	const sint32 direction = is_forward ? 1 : -1;
