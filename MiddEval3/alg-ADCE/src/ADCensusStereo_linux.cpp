...
* Describe	: implement of ad-census stereo class (Linux version) - Modified to match standard version
*/
...
bool ADCensusStereo::Match(const uint8* img_left, const uint8* img_right, float32* disp_left)
{
	...
	// 输出视差图 - 修改：使用height_ * width_与普通版本一致
...
void ADCensusStereo::ScanlineOptimize()
{
	...
	// 设置参数
	scan_line_.SetParam(width_, height_, option_.min_disparity, option_.max_disparity, 
	                   option_.so_p1, option_.so_p2, option_.so_tso);
	// 扫描线优化
	scan_line_.Optimize();
}
...
// 修改：完整实现ComputeDisparity，与普通版本完全一致
...
// 修改：完整实现ComputeDisparityRight，与普通版本完全一致
